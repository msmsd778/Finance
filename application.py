...
def index():
    ...
    for row in rows:
        ...
        holdings.append({"symbol": stock["symbol"], "name": stock["name"], "shares": row["totalShares"], "price": stock["price"], "total": stock["price"] * row["totalShares"]})
...
def buy():
    ...
    if request.method == "POST":
        ...
        if newcash < 0:
            return apology("You cannot afford this share")
...
def quote():
    ...
    if request.method == "POST":
        if not request.form.get("symbol"):
            return apology("symbol cannot be empty", 403)
        symbol = request.form.get("symbol").upper()
        stock = lookup(symbol)
        if stock is None:
...
def register():
    if request.method == "POST":
        if not request.form.get("username"):
            ...
            return apology("must confirm password correctly", 403)
